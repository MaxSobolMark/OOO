...
def multiple_action_q_function(forward):
    ...
    def wrapped(self, observations, actions, **kwargs):
        ...
        q_values, penultimate_layer_activations = forward(
            self, observations, actions, **kwargs
        )
        if multiple_actions:
            q_values = q_values.reshape(batch_size, -1)
        return q_values, penultimate_layer_activations
...
class FullyConnectedNetwork(nn.Module):
    ...
    @nn.compact
    def __call__(self, input_tensor) -> Tuple[jnp.ndarray, jnp.ndarray]:
        x = input_tensor
        ...
        penultimate_layer_activations = x
        if self.feature_normalization:
            denominator = jnp.linalg.norm(x, axis=-1, keepdims=True)
            # clamp min to 1e-8 to avoid dividing by zero
        ...
        return output, penultimate_layer_activations
...
class FullyConnectedQFunction(nn.Module):
    ...
    def __call__(self, observations, actions):
        ...
        x, penultimate_layer_activations = FullyConnectedNetwork(
            output_dim=1,
            arch=self.arch,
            orthogonal_init=self.orthogonal_init,
            feature_normalization=self.feature_normalization,
        )(x)
        return jnp.squeeze(x, -1), penultimate_layer_activations
...
class TanhGaussianPolicy(nn.Module):
    ...
    def log_prob(self, observations, actions):
        ...
        base_network_output, _ = self.base_network(observations)
